...
#Geração de regras seguindo algoritmo Apriori - Busca da melhor dupla (Assim descobrimos que a maior vencedora é a ANA)
ap <- apriori(final, parameter = list(support=0.1, confidence=0.6), appearance = NULL, control = NULL)
#inspect(ap)
sub <- subset(ap, (rhs %in% "Resultado=1"))
inspect(sort(sub))

#Geração de regras - Busca da melhor dupla (Assim descobrimos que a dupla é Ana e Agata)
sub <- subset(ap, (rhs %in% "Resultado=1") & (lhs %in% "Ana=1"))
inspect(sort(sub))
...
ap <- apriori(final, parameter = list(support=0.01, confidence=0.4), appearance = NULL, control = NULL)
sub <- subset(ap, (rhs %in% "Resultado=1") & (lhs %in% "Ana=1") & (lhs %in% "agata=1") )
inspect(sort(sub))

####################### DESCOBERTA DA PIOR EQUIPE ########################

#Geração de regras seguindo algoritmo Apriori - Busca do pior jogador (Assim descobrimos que o fabio é o que mais perdeu)
ap <- apriori(final, parameter = list(support=0.1, confidence=0.4), appearance = NULL, control = NULL)
sub <- subset(ap, (rhs %in% "Resultado=0"))
inspect(sort(sub))

# Busca da pior dupla (Assim descobrimos o segundo integrante é a Barbara)
sub <- subset(ap, (rhs %in% "Resultado=0") &  (lhs %in% "Fabio=1"))
inspect(sort(sub))

#Geração de regras seguindo algoritmo Apriori - Baseado na dupla, busca do ultimo integrante da equipe (Assim descobrimos o terceiro integrante é a Ana)
ap <- apriori(final, parameter = list(support=0.01, confidence=0.4), appearance = NULL, control = NULL)
sub <- subset(ap, (rhs %in% "Resultado=0") & (lhs %in% "Fabio=1") & (lhs %in% "Barbara=1") )
inspect(sort(sub))
